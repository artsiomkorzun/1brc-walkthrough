...
public class Challenge_12_SwarTemperature implements Challenge {
    ...
    public static void main(String[] args) {
        new Challenge_12_SwarTemperature().run(args);
    }
    ...
    static void loop(long position, long limit, Aggregates aggregates) {
        while (position < limit) {
            ...
            {  // 8-byte SWAR parsing for temperatures: -##.#, -#.#, #.#, ##.#
                ...
                temperature = (int) ((abs ^ signed) - signed);
                position += (dot >> 3) + 3;
            }
    ...
    static class Aggregates { // keys <= 10K, no need to grow
        ...
        Entry put(long position, int length, long hash, long tail) {
            ...
            for (int mask = entries.length - 1, index = mix(hash) & mask; ; index = (index + 1) & mask) {
