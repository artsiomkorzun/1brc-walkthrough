...
public class Challenge_08_UnsafeParsing implements Challenge {
    ...
    public static void main(String[] args) {
        new Challenge_08_UnsafeParsing().run(args);
    }
    ...
    static void loop(long position, long limit, Key key, Aggregates aggregates) {
    ...
    static class Aggregator extends Thread {
        ...
        public void run() {
            ...
            Key key = new Key();
            loop(position, limit, key, aggregates);
    ...
    static class Aggregates { // keys <= 10K, no need to grow
        ...
        Aggregate put(Key key) {
            ...
            for (int mask = keys.length - 1, index = key.hash & mask; ; index = (index + 1) & mask) {
