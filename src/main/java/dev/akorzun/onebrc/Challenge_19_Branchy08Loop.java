...
public class Challenge_19_Branchy08Loop implements Challenge {
    ...
    public static void main(String[] args) {
        new Challenge_19_Branchy08Loop().run(args);
    }
    ...
    static void loop(long position, long limit, Aggregates aggregates) {
        while (position < limit) {
            ...
            if (separator != 0) {
                ...
                long word0 = word;
                word = UNSAFE.getLong(position + 8);
                separator = comma(word);
                ...
                if (separator != 0) {
                    ...
                    while (true) {
                        word = UNSAFE.getLong(position + length);
                        separator = comma(word);
    ...
    static class Aggregates {
        ...
        long find(long word, long hash) {
            long address = pointer + offset(hash);
            long w = UNSAFE.getLong(address + 24);
